...
- Current character reference:
  `assets/reference/hero_new_model_ref_asym_leg_clean.png`
...
- `assets/grids/sprite_reference_grid_16f_256_4x4.png`
- 4 rows by 4 columns
...
All generated sources should pass through `tools/animation_pipeline.py`.
...
For external atlas sources, use `--take-first` to copy only the first N cells in
row-major order. This preserves the current `FancyOrkin-run.png` workflow:
...
Do not promote raw generated files directly into a game. Promote only cleaned
pipeline outputs.
...
Current example promoted outputs:

- `examples/outputs/hero_new_run_sheet_11f_256.png`
- `examples/outputs/hero_new_jump_sheet_16f_256.png`

When promoted sheets are copied into a game, update any cache keys and animation
metadata in that game.

If a frame's gameplay meaning changes, update timing/selection logic in the
consuming game at the same time. The current example jump sheet expects frames
1-6 for liftoff, frame 11 for falling, and frames 12-16 for landing rather than
being played as a blind loop.
