...
"""
Canonical cleaner/repacker for 2D animation sprite-sheet sources.

Raw image-generation output is never a finished game sheet. Treat generated or
third-party sheets as rough pose sources that must be normalized here before
promotion into a game.
...
def run(args: argparse.Namespace) -> dict[str, object]:
    ...
    if args.split_mode == "components":
        ...
        if args.take_first is None and args.grid_cols * args.grid_rows != args.frames:
            raise ValueError("--grid-cols * --grid-rows must equal --frames unless --take-first is set")
        take_first = args.take_first if args.take_first is not None else args.frames
        if take_first != args.frames:
        ...
        if len(cells) != args.frames:
            raise ValueError(f"Expected {args.frames} grid cells, got {len(cells)}")
    else:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Clean, repack, and validate 2D animation sprite-sheet sources.")
